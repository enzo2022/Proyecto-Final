...
const createUser = async (req, res) => {
	try {
		const { email, userName, password, photo, cellphone, user_type } =
			req.body;
		...
		if (!userName || !email || !password)
			return res
				.status(412)
				.send("Users, Passd y Email no pueder ser nulos");
...
const prueba = (req, res) => {
	try {
		res.status(200).json(
			"ACCESO PERMITIDO PEUS ERES=> SOLO PEUDEN ENTRAR PREMIUN Y ADMIN"
		);
	} catch (err) {
